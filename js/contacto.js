...
function jGallery_move(id, cnt, dir, w, visible, transitionInterval) {
  ...
  if (dir > 0) {
    ...
    if (curr + cnt * w - visible * w <= 0) return false;
  }
...
function jMenu_open() {
  ...
  if ($("a", this).html() == jMenu_openid) return;

  if (jMenu_action) return;

  jMenu_close();

  if ($("ul", this).size() == 0) return;

  jMenu_action = true;
  jMenu_ddmenuitem = $(this)
    .find("ul")
    .slideDown(jMenu_effectTime, function() {
      jMenu_action = false;
    });
